"""Defines the abstract interface for Large Language Models (LLMs)
used in the Explainability Module.

This abstraction allows swapping out different LLM providers
(e.g., Google Gemini, OpenAI GPT).
"""
...
class LLMInterface(ABC):
    ...
    async def generate_explanation(
        ...
        """Generate a human-readable explanation and a structured JSON explanation from an LLM.
        ...
        Returns:
            Tuple[str, Dict[str, Any]]: A tuple containing the human-readable
                                         explanation (string) and the structured
    ...
    async def _extract_structured_json(self, llm_response: str) -> Dict[str, Any]:
        """(Internal) Extract structured JSON from a raw LLM response.
...
class MockLLM(LLMInterface):
    ...
    async def generate_explanation(
        ...
        """Generate a mock explanation and structured JSON."""
        print(
            f"MockLLM: Generating explanation for prompt (truncated): {prompt[:100]}..."
        )
        human_readable = f"This is a mock explanation for: {prompt}"
        structured_json = {
            "mock_explanation": "true",
