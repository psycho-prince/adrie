...
from app.core.models import SimulateRequest, SimulateResponse, PlanRequest, PlanResponse, ExplanationType
from app.api.main import adrie_instances # To check internal state
from app.core.models import Mission, MissionStatus, AgentType, AgentCapability, Coordinate
from app.environment.engine import EnvironmentEngine
import numpy as np # For random choices in test setup
...
@pytest.mark.asyncio
async def test_simulate_disaster_success(test_app: AsyncClient):
    ...
    assert response_data.mission_id in adrie_instances
    mission_state = adrie_instances[response_data.mission_id]
    assert isinstance(mission_state["environment_engine"], EnvironmentEngine)
    assert mission_state["mission"].status == MissionStatus.IN_PROGRESS
    assert len(mission_state["environment_engine"].get_all_victims()) == request_payload["num_victims"]
    assert len(mission_state["agent_coordination_engine"].get_all_agents()) == request_payload["num_agents"]

@pytest.mark.asyncio
async def test_simulate_disaster_conflict(test_app: AsyncClient):
...
@pytest.mark.asyncio
async def test_plan_generation_success(test_app: AsyncClient):
...
@pytest.mark.asyncio
async def test_plan_generation_mission_not_found(test_app: AsyncClient):
...
@pytest.mark.asyncio
async def test_get_metrics_success(test_app: AsyncClient):
...
@pytest.mark.asyncio
async def test_get_metrics_mission_not_found(test_app: AsyncClient):
...
@pytest.mark.asyncio
async def test_get_explain_victim_prioritization_success(test_app: AsyncClient):
    ...
    mission_state = adrie_instances[mission_id]
    env_engine: EnvironmentEngine = mission_state["environment_engine"]
    victims = env_engine.get_all_victims()
...
@pytest.mark.asyncio
async def test_get_explain_mission_summary_success(test_app: AsyncClient):
...
@pytest.mark.asyncio
async def test_get_explain_mission_not_found(test_app: AsyncClient):
...
@pytest.mark.asyncio
async def test_get_explain_victim_not_found(test_app: AsyncClient):
...
@pytest.mark.asyncio
async def test_get_explain_missing_decision_id_for_prioritization(test_app: AsyncClient):
