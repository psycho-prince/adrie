"""
The Metrics Engine is responsible for tracking, calculating, and reporting
...
from app.core.models import Mission, MetricsSummary, MetricType
...
class MetricsEngine:
    ...
    def __init__(self, mission_id: UUID):
        """
        Initializes the MetricsEngine for a specific mission.

        Args:
            mission_id (UUID): The ID of the mission this metrics engine is tracking.
        """
        self.mission_id = mission_id
        self.metrics_data: Dict[str, Any] = {} # Raw data collected during mission
        logger.info(f"MetricsEngine initialized for mission {self.mission_id}.")

    async def record_metric(self, metric_type: MetricType, value: Any, timestamp: Optional[str] = None) -> None:
        """
        ...
        logger.debug(f"Recorded metric {metric_type.value}: {value} for mission {self.mission_id}.")
    ...
    async def get_metrics_summary(self, mission: Mission) -> MetricsSummary:
        ...
        if mission.start_time and mission.end_time:
            ...
            summary_data[MetricType.TOTAL_RESCUE_TIME.value] = int((end - start).total_seconds())
        else:
             summary_data[MetricType.TOTAL_RESCUE_TIME.value] = None # Still ongoing

        # Victims Rescued Count
        summary_data[MetricType.VICTIMS_RESCUED_COUNT.value] = len(mission.victims_rescued)

        # Predicted Lives Saved (Placeholder)
        # This would typically come from the Victim Prioritization Model or simulation outcomes
        summary_data[MetricType.PREDICTED_LIVES_SAVED.value] = len(mission.victims_rescued) # Simple for now
        ...
        if MetricType.AGGREGATE_RISK_EXPOSURE.value in self.metrics_data and 
           len(self.metrics_data[MetricType.AGGREGATE_RISK_EXPOSURE.value]) > 0:
            risk_values = [item["value"] for item in self.metrics_data[MetricType.AGGREGATE_RISK_EXPOSURE.value]]
            summary_data["average_agent_risk_exposure"] = sum(risk_values) / len(risk_values)
        else:
        ...
        try:
            return MetricsSummary(
                ...
                total_rescue_time_seconds=summary_data.get(MetricType.TOTAL_RESCUE_TIME.value),
                average_agent_risk_exposure=summary_data.get("average_agent_risk_exposure"),
                agent_utilization_percentage=summary_data.get("agent_utilization_percentage"),
                efficiency_index=summary_data.get("efficiency_index"),
                predicted_lives_saved=summary_data.get(MetricType.PREDICTED_LIVES_SAVED.value),
                victims_rescued_count=summary_data.get(MetricType.VICTIMS_RESCUED_COUNT.value),
                active_agents_count=summary_data.get("active_agents_count")
            )
        except Exception as e:
            logger.error(f"Error creating MetricsSummary for mission {self.mission_id}: {e}", exc_info=True)
            raise

    def reset(self) -> None:
        """Resets the metrics engine, clearing all collected data."""
        self.metrics_data = {}
        logger.info(f"MetricsEngine for mission {self.mission_id} reset.")
